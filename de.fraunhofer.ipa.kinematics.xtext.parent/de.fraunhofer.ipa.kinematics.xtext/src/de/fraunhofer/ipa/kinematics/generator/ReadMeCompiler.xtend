...
class ReadMeCompiler {
	...
	def compile_ReadMe(Component component) '''
		...
		The package structure of «component.gitRepo.package» is as follows
		- `./urdf/` - holds the URDF file(s)
		- `./models/` - holds the kinematic model file(s) of `«component.name»` and its dependent models 
		
		## Installation (Ubuntu)
		To make installation easy, we will install the required packages using `vcs` and a `«component.name».repos` file. `«component.name».repos` can be found at the root of `«component.gitRepo.package»` package.
		```bash
		cd ~/ros2_ws/src
		...
		colcon test --packages-select «component.gitRepo.package»
		...
		```bash
		ros2 launch «component.gitRepo.package» display.launch.py
		```
	'''
}
